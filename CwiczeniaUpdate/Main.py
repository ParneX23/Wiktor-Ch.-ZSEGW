...
def aktualizuj_ilosc(id_itemu, nowa_ilosc):
    conn = polacz_z_baza()
    if conn is None: return

    cursor = conn.cursor()
    sql = "UPDATE produkty SET ilosc=%s WHERE id=%s"
    val = (nowa_ilosc, id_itemu)

    try:
        cursor.execute(sql, val)
        conn.commit()
        messagebox.showinfo("Sukces", "Ilość zmieniona pomyślnie.")
    except mysql.connector.Error as err:
        messagebox.showerror("Błąd", f"Wystąpił błąd podczas zmiany ilości: {err}")
    finally:
        cursor.close()
        conn.close()
...
def obsluga_aktualizacji(nazwa_var, ilosc_var, tree):
    nazwa = nazwa_var.get()
    try:
        ilosc = int(ilosc_var.get())
    except ValueError:
...
ttk.Label(form_frame, text="Id Produktu:").grid(row=0, column=0, padx=5, pady=5, sticky='w')
ttk.Entry(form_frame, textvariable=id_produktu_var, width=30).grid(row=0, column=1, padx=5, pady=5)

ttk.Label(form_frame, text="Nazwa Produktu:").grid(row=1, column=0, padx=5, pady=5, sticky='w')
ttk.Entry(form_frame, textvariable=nazwa_produktu_var, width=30).grid(row=1, column=1, padx=5, pady=5)

ttk.Label(form_frame, text="Ilość:").grid(row=2, column=0, padx=5, pady=5, sticky='w')
ttk.Entry(form_frame, textvariable=ilosc_var, width=10).grid(row=2, column=1, padx=5, pady=5, sticky='w')

dodaj_btn = ttk.Button(form_frame, text="Dodaj Produkt",
    command=lambda: obsluga_dodawania(nazwa_produktu_var, ilosc_var, tree))
dodaj_btn.grid(row=3, column=0, pady=10)

update_btn = ttk.Button(form_frame, text="Aktualizuj Ilosc",
    command=lambda: obsluga_aktualizacji(id_produktu_var, ilosc_var, tree))
